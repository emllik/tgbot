...
class http_session(object): 
    ...
    def create_connection(self):
        ...
        sock = contex.wrap_socket(socket.socket(), server_hostname=host)
    ...
    def post(self, url, data):
        ...
        try: 
            ...
            sock.send(header.encode()+request)
...
class bot(object):
    ...
    def sendVoice(self, chat_id, file, caption='', reply_markup=None, reply_to_msg_id=None, parse_mode=None):
        url = 'sendVoice'
        data = { 'chat_id': str(chat_id), 'voice': file, 'caption': str(caption)}
        if reply_markup:
             data['reply_markup'] = reply_markup
        if reply_to_msg_id:
             data['reply_to_message_id'] = reply_to_msg_id
    ...
    def downloadFile(self,  file_id):
        ...
        sock = self.session.connections.pop()
        sock.send(header.encode())
        data = self.session.recv(sock)
        self.session.connections.add(sock)
        return data
